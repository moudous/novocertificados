...
<body>
<header class="page-info bg-white border-bottom"><div class="container-fluid py-3"><div class="d-flex flex-wrap justify-content-between align-items-center gap-3"><div><h1 class="h4 mb-2">Certificado</h1><div><strong>Participante:</strong> {{ $item->participante?->nome ?: '—' }}</div><div><strong>Evento:</strong> {{ $item->novoCertificado?->evento?->nome ?: $item->novoCertificado?->atividade?->evento?->nome ?: '—' }}</div><div><strong>Atividade:</strong> {{ $item->novoCertificado?->atividade?->nome ?: '—' }}</div><div><strong>Horas:</strong> {{ data_get($item->snapshot_dados,'atividade.carga_horaria') ?: '—' }}</div></div><div class="text-md-end"><button type="button" class="btn btn-primary" onclick="window.print()">Visualizar impressão</button><div class="small text-muted mt-2">A página já está configurada em A4 {{ $landscape ? 'paisagem' : 'retrato' }}, sem margens.<br>Se o navegador oferecer a opção, desative “Cabeçalhos e rodapés”.</div></div></div></div></header>
