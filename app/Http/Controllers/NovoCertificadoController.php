...
class NovoCertificadoController extends Controller
{
    ...
    public function generateParticipant(Request $request, NovoCertificado $certificado, ListaParticipante $item, TemplateLayoutRenderer $renderer, PdfDigitalSigner $signer): RedirectResponse|JsonResponse
    {
        ...
        if($request->expectsJson())return response()->json(['html'=>'<a target="_blank" rel="noopener noreferrer" href="'.e(route('certificadosnovos.public.pdf',$item->fresh()->codigo)).'" class="btn btn-sm btn-outline-danger listagem-acao" title="Abrir PDF" aria-label="Abrir PDF"><i class="bi bi-file-earmark-pdf-fill"></i></a>']);
    ...
    private function generateItem(NovoCertificado $certificado, ListaParticipante $item, TemplateLayoutRenderer $renderer, PdfDigitalSigner $signer): void
    {
        ...
        $hours=data_get($item->dados_personalizados,'carga_horaria',$certificado->carga_horaria);
        $context=['participante'=>['nome'=>$item->participante?->nome,'email'=>$item->participante?->email,'cpf'=>$item->participante?->cpf],'evento'=>['nome'=>$certificado->evento?->nome,'descricao'=>$certificado->evento?->descricao],'atividade'=>['nome'=>$certificado->atividade?->nome,'carga_horaria'=>$hours],'eventos'=>['periodos'=>$certificado->evento?->periodos],'atividades'=>['periodos'=>$certificado->atividade?->periodos],'responsavel'=>['nome'=>$responsavel?->participante?->nome,'cargo'=>$responsavel?->cargo,'titulacao'=>$responsavel?->titulacao,'rubrica_path'=>$renderer->rubricaPath($rubrica)],'emissao'=>['nome'=>$certificado->nome,'data'=>($certificado->data_emissao?:now())->format('d/m/Y'),'carga_horaria'=>$hours],'certificado'=>['codigo'=>$code],'template'=>$item->dados_personalizados??[],'link_validacao'=>route('certificadosnovos.public.pdf',$code)];
