...
class NovoCertificadoController extends Controller
{
    ...
    {
        $ownOnly=$this->authorizeParticipantInsertion($request);$userId=$this->sessionUserId($request);$data=$request->validate(['participantes'=>['required','array','min:1'],'participantes.*'=>['integer','distinct',Rule::exists('participantes','id')->where(fn($q)=>$q->whereNull('excluido_em')->when($ownOnly,fn($q)=>$q->where('criado_por',$userId)))]]);DB::transaction(function()use($certificado,$data,$userId){foreach($data['participantes'] as $id)$certificado->participantes()->firstOrCreate(['participante_id'=>$id],['adicionado_por'=>$userId]);$certificado->update(['lista_participantes_id'=>$certificado->participantes()->min('id')]);}); return back()->with('status','Participantes adicionados com sucesso.');
    }
